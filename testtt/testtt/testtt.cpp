...
{
	if (!mask.empty())  //所有遮罩设置为背景
		mask.setTo(Scalar::all(GC_BGD));
	bgdPxls.clear(); fgdPxls.clear();       //矩形的顶点清空
...
int main(int argc, char** argv)
{
	...
	string filename = "D:\\Study\\UNI\\labs\\tokyo mood\\pics\\72.png";
	Mat image0 = imread(filename, 1);
	Mat image;
	resize(image0, image, Size(), 1, 1);
	if (image.empty())
	{
	...
	for (;;)
	{
		...
		case 'r':
			...
			break;
		case 'n':
			int iterCount = gcapp.getIterCount();  //获得操作次数
			cout << "<" << iterCount << "... ";
